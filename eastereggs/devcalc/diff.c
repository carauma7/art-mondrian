/***********************************************************************
 * 
 * diff.c | "Função pertencente ao cálculo da diferenciação."
 * 
...
char *differentiate(char *str, int mode) {// o modo determina quando ser recursivo
    /* preserva a str original */
    char *str_cpy = (char *) malloc(sizeof(char) * MAX_CHAR / 8);
    strcpy(str_cpy, str);

    /* neutraliza uma entidade de parênteses-fechados que não é composto */
    ...
    if ((num_tm == 1) && (num_bl == 1)) {
        if ((mode == 1) && (is_composite(str_cpy))) {
            ...
            /* converte em derivada */
            return fn_diff(str_cpy);
        }
    } else if (num_tm != 1) {
        term *tm = into_term(str_cpy);// nos termos
        bool op = false;

        strcpy(rt_str, "(");
        while (tm->segm != NULL) {
            if (!op) {
                /* dividir ainda mais ou apelar */
                ...
                strcat(rt_str, tm->segm->entry);// operador armazenado
                op = false;
            }
        ...
        list *mult_head = bl->mult; // aplica regra do produto
        list *mult_curr = mult_head; 
        list *divi_head = bl->divi;  // aplica regra da divisão
        ...
        /* remove todos os parênteses fechados */
        ...
        if (n_divi == 0) {// sem regra da divisão
            strcpy(rt_str, "(");
    ...
    return NULL; // combinação de num_tm/num_bl não tratada
}
...
char *fn_diff(char *str) {
    ...
    if (fn_tp == cnst) {
        ...
        *(pt++) = 0;
        ...
        char *rt_str = (char *) malloc(sizeof(char) * MAX_CHAR / 8);
        if (id_ch_tp(str_cpy[0]) == pt_sig) {
            if (str_cpy[0] == '-') {                       // possui um sinal de menos "-"
                if (exp < 0) {
                    strcpy(rt_str, "(");
                    if ((exp * -1) != 1) {                 // desnecessário para coeficiente de 1
            ...
            } else if (str_cpy[0] == '+') { // possui sinal de soma "+"
                if (exp == 0) {
                    return "(0)";
                } else if (exp == 1) {
                    return "(1)";
        ...
        *(pt++) = 0;
        ...
        if (strcmp(pt, "1") == 0) {
            if (strcmp(str_cpy, "x") == 0) {// não composta
                return "(1)";
            } else {// composta
                return "";
            }
    ...
    return NULL; // tipo de função não reconhecido
}
